...
function InnerwarMatchesContent() {
  const params = useParams();
  const router = useRouter();
  const userId = params.userId as Id<"users">;

  const currentUser = useQuery(api.users.getCurrentUser);
  const stats = useQuery(api.innerwars.getInnerwarPlayerStats, { userId });
  ...
  const filtered = useMemo(() => {
    ...
    return matches.filter((m) => m.innerwar?.year === Number(effectiveYear));
...
export default function PlayerInnerwarMatchesPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">
